const _MINIMUM_NAME_LENGTH = 3;
const _MAXIMUM_AGE = 120;
export default class Person {
  ...
  // protected MAXIMUM_AGE = 120;
    ...
    private validateName(name: string): void {
      if (name.length < _MINIMUM_NAME_LENGTH) {
        throw new Error(`O nome deve conter no mínimo ${_MINIMUM_NAME_LENGTH} caracteres.`);
      }
    }
    ...
    private validateBirthDate(date: Date): void {
      ...
      if (Person.getAge(date) > _MAXIMUM_AGE) {
        throw new Error(`A pessoa deve ter no máximo ${_MAXIMUM_AGE} anos.`);
      }
    }
