...
function Favorites() {
  ...
  const languages = favorites.map((item) => item.language).filter((notNull) => notNull);
  const uniqueLanguages = [...new Set(languages.sort())];

  const filteredByName = favorites
  ...
  return (
    <div className="Home">
      ...
      <h1>{`Repositórios ${filteredByName.length} de ${favorites.length}`}</h1>
